      ...
      if (userId != null) {
        ...
        switch (document.documentType.toUpperCase()) {
          case 'FE': // Factura Electrónica
            accountingRecorded = await ElectronicInvoicingIntegrationService
                .recordElectronicInvoice(
          ...
          case 'NC': // Nota Crédito
            accountingRecorded =
                await ElectronicInvoicingIntegrationService.recordCreditNote(
              document: updatedDocument,
              userId: userId,
              cashSessionId: openSession?.id,
          ...
          case 'ND': // Nota Débito
            accountingRecorded =
                await ElectronicInvoicingIntegrationService.recordDebitNote(
              document: updatedDocument,
              userId: userId,
              cashSessionId: openSession?.id,
              reason: document.observations,
          ...
          default:
            print(
                '⚠️ Tipo de documento no reconocido para integración contable: ${document.documentType}');
        }
  ...
  static Future<DocumentGenerationResult> generateAllDocuments({
    ...
    try {
      ...
      final jsonPath = path.join(
          outputDir, 'json', '${document.documentNumber}_$timestamp.json');
      final pdfPath = path.join(
          outputDir, 'pdf', '${document.documentNumber}_$timestamp.pdf');
      final xmlPath = path.join(
          outputDir, 'xml', '${document.documentNumber}_$timestamp.xml');
