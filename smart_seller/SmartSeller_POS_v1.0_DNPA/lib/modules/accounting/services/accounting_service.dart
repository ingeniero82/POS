...
class AccountingService {
  ...
  static const String _transactionCategoriesTableName =
  ...
  static Future<List<AccountingEntry>> getAccountingEntriesByDate(
      DateTime date) async {
    try {
      final db = SQLiteDatabaseService.database;
      if (db == null) throw Exception('Base de datos no inicializada');
      ...
      return List.generate(
          maps.length, (i) => AccountingEntry.fromMap(maps[i]));
    } catch (e) {
  ...
  static Future<Map<String, double>> getIncomeExpenseSummary(
      DateTime date) async {
    try {
      final db = SQLiteDatabaseService.database;
      if (db == null) throw Exception('Base de datos no inicializada');
  ...
  static Future<void> _createCashMovementFromEntry(
      AccountingEntry entry, int entryId) async {
    try {
  ...
  static Future<List<CashMovement>> getCashMovementsBySession(
      int sessionId) async {
    try {
      final db = SQLiteDatabaseService.database;
      if (db == null) throw Exception('Base de datos no inicializada');
  ...
  static Future<void> closeCashSession(
      int sessionId, double finalAmount, int userId,
      {String? notes}) async {
    try {
      final db = SQLiteDatabaseService.database;
      if (db == null) throw Exception('Base de datos no inicializada');
      ...
      final theoreticalBalance =
          session.initialAmount + totalIncome - totalExpense;
  ...
  static Future<List<TransactionCategory>> getCategoriesByType(
      String type) async {
    try {
      final db = SQLiteDatabaseService.database;
      if (db == null) throw Exception('Base de datos no inicializada');
      ...
      return List.generate(
          maps.length, (i) => TransactionCategory.fromMap(maps[i]));
    } catch (e) {
  ...
  static Future<void> recordSaleIncome(
      double amount, String description, int userId,
      {String? paymentMethod, String? reference}) async {
    try {
      final openSession = await getOpenCashSession();
      if (openSession == null) {
  ...
  static Future<void> recordExpense(
      double amount, String description, int userId,
      {String? category, String? paymentMethod, String? reference}) async {
    try {
      final openSession = await getOpenCashSession();
      if (openSession == null) {
        throw Exception('No hay sesión de caja abierta');
  ...
  static Future<void> recordSupplierPayment(
      double amount, String supplierName, int userId,
  ...
  static Future<void> recordSupplierReturn(
      double amount, String supplierName, int userId,
      {String? paymentMethod,
      String? reference,
      String? documentNumber}) async {
