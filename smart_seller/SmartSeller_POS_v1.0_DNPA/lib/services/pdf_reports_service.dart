...
class PDFReportsService {
      ...
      pw.MultiPage(
        ...
        build: (pw.Context context) {
          return [
            ...
            if ((report.totalDiscounts ?? 0) > 0 || (report.totalReturns ?? 0) > 0) ...[
              _buildDiscountsAndReturns(report),
              pw.SizedBox(height: 20),
            ],
            
            // ✅ NUEVO: Ingresos adicionales (si hay)
            if (report.additionalIncomes != null && report.additionalIncomes!.isNotEmpty) ...[
              _buildAdditionalIncomes(report.additionalIncomes!),
              pw.SizedBox(height: 20),
            ],
            
            // ✅ NUEVO: Egresos detallados (si hay)
            if (report.expenseDetails != null && report.expenseDetails!.isNotEmpty) ...[
              ...
              pw.Container(
                ...
                child: pw.Column(
                  ...
                  children: [
                    ...
                    _buildCashRow('Saldo Inicial:', report.initialBalance ?? 0.0),
                    _buildCashRow('Saldo Teórico:', report.theoreticalBalance ?? 0.0),
                    _buildCashRow('Saldo Real:', report.actualBalance ?? 0.0),
                    if (report.cashDifference != null && report.cashDifference! != 0.0)
                      _buildCashRow(
                        'Diferencia:', 
  ...
  static pw.Widget _buildCashRow(String label, double amount, {bool isDifference = false}) {
    return pw.Padding(
      ...
      child: pw.Row(
        ...
        children: [
          ...
          pw.Text(
            ...
            style: pw.TextStyle(
              fontSize: 10,
              fontWeight: isDifference ? pw.FontWeight.bold : pw.FontWeight.normal,
            ...
            children: [
              ...
              ...incomes.map((income) => pw.TableRow(
                children: [
                  pw.Padding(
                    padding: const pw.EdgeInsets.all(6),
                    child: pw.Text(income.description, style: const pw.TextStyle(fontSize: 9)),
                  ),
                  pw.Padding(
                    padding: const pw.EdgeInsets.all(6),
                    child: pw.Text(income.category, style: const pw.TextStyle(fontSize: 9)),
                  ),
                  pw.Padding(
                    padding: const pw.EdgeInsets.all(6),
                    child: pw.Text(_currencyFormat.format(income.amount), style: const pw.TextStyle(fontSize: 9)),
                  ),
                ],
              )),
            ],
            ...
            children: [
              if (totalDiscounts > 0) ...[
                pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    pw.Text('Descuentos:', style: pw.TextStyle(fontSize: 10, color: PdfColors.grey700)),
                    pw.Text(_currencyFormat.format(totalDiscounts), style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold, color: PdfColors.orange700)),
                  ],
                ),
              ],
              if (totalReturns > 0) ...[
                pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    pw.Text('Devoluciones:', style: pw.TextStyle(fontSize: 10, color: PdfColors.grey700)),
                    pw.Text(_currencyFormat.format(totalReturns), style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold, color: PdfColors.red700)),
                  ],
                ),
              ],
              if (returnTransactions > 0) ...[
                pw.Column(
                  crossAxisAlignment: pw.CrossAxisAlignment.start,
                  children: [
                    pw.Text('Transacciones Devoluidas:', style: pw.TextStyle(fontSize: 10, color: PdfColors.grey700)),
                    pw.Text('$returnTransactions', style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold, color: PdfColors.red800)),
                  ],
                ),
              ],
            ],
            ...
            children: [
              ...
              ...expenses.map((expense) => pw.TableRow(
                children: [
                  pw.Padding(
                    padding: const pw.EdgeInsets.all(6),
                    child: pw.Text(expense.description, style: const pw.TextStyle(fontSize: 9)),
                  ),
                  pw.Padding(
                    padding: const pw.EdgeInsets.all(6),
