...
class AuthorizationService {
  ...
  Future<bool> _verifyUserCodeAuthorization(User user, String action) async {
    try {
      ...
      if (!permissionsService.hasPermission(
          user.role, Permission.allowUserCode)) {
      ...
      switch (action) {
        case PRICE_CHANGE:
          return permissionsService.hasPermission(
              user.role, Permission.modifyPrice);

        case WEIGHT_MANUAL:
          return permissionsService.hasPermission(
              user.role, Permission.manualWeight);
  ...
  Future<void> logAuthorization(
      String action, String authorizedBy, String details) async {
    try {
      final currentUser = AuthService.to.currentUser;
      ...
      print(
          'AUDITORÍA: $action - Autorizado por: $authorizerInfo - Usuario: ${currentUser?.fullName} - Fecha: $timestamp - Detalles: $details');
