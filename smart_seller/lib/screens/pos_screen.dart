...
class _PosScreenState extends State<PosScreen> {
  ...
  List<Product> _frequentProducts =
      []; // ✅ NUEVO: Productos más frecuentes basados en ventas
  ...
  Future<void> _loadFrequentProducts() async {
    try {
      ...
      final frequentProductsData =
          await SQLiteDatabaseService.getMostFrequentProductNames(
        days: 30,
        limit: 12,
      );
      ...
      for (final productData in frequentProductsData) {
        ...
        if (unit.isNotEmpty) {
          ...
          final exactMatch =
              _products.where((p) => p.name == name && p.unit == unit).toList();
          ...
          children: [
            ...
            TextField(
              controller: _currentMode == 'barcode'
                  ...
                  : _quantityController,
              focusNode:
                  _currentMode == 'barcode' ? _barcodeFocus : _quantityFocus,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
                'Precio: ${CurrencyFormatter.formatCurrency(_selectedProduct!.price)}'),
            ...
            TextField(
              ...
              onSubmitted: (value) async =>
                  await _addToCart(int.tryParse(value) ?? 1),
            ),
            const SizedBox(height: 16),
          ...
          children: [
            ...
            Obx(() {
              if (_posController.currentClient != null) {
                ...
                final validationSummary =
                    ClientValidationService.getClientValidationSummary(client);
                final canReceiveInvoice =
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      ...
                      subtitle: Text(
                          '${item.unit} x ${CurrencyFormatter.formatCurrency(item.price)}'),
                      trailing: Text(
            ...
            if (_posController.currentClient != null) ...[
              ...
              Expanded(
                child: ElevatedButton.icon(
                  ...
                  label: const Text('Limpiar Cliente',
      ...
      switch (event.logicalKey.keyLabel) {
        ...
        case 'Enter':
          ...
          if (_selectedProduct != null && !_quantityFocus.hasFocus) {
            _addToCart(int.tryParse(_quantityController.text) ??
                1); // ✅ Async - no necesita await aquí
          }
          break;
      }
    }
          ...
          void updateSearchResults(String query) {
            if (query.isEmpty) {
              ...
              final exactMatch = _products
                  .where((p) => p.code == query || p.shortCode == query)
                  .toList();
              ...
              if (exactMatch.isNotEmpty) {
                ...
                searchResults = _products
                    .where((p) => quitarTildes(p.name.toLowerCase())
                        .contains(quitarTildes(query.toLowerCase())))
                    .take(20)
                    .toList(); // Limitar a 20 resultados
              }
            }
            setDialogState(() {});
          }
            ...
            content: SizedBox(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildVirtualTextKeyboard(
                      searchController, setDialogState, updateSearchResults),
                ],
              ),
            ),
            actions: [
        ...
        children: [
            ...
            children: [
              Expanded(
                  child: _buildTextButton(
                      'Q', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'W', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'E', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'R', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'T', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'Y', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'U', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'I', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'O', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'P', controller, setState, onTextChanged)),
            ],
          ),
          const SizedBox(height: 4),
          ...
          Row(
            children: [
              Expanded(
                  child: _buildTextButton(
                      'A', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'S', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'D', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'F', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'G', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'H', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'J', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'K', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'L', controller, setState, onTextChanged)),
            ],
          ),
          const SizedBox(height: 4),
          ...
          Row(
            children: [
              Expanded(
                  child: _buildTextButton(
                      'Z', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'X', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'C', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'V', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'B', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'N', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      'M', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      '0', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      '1', controller, setState, onTextChanged)),
            ],
          ),
          const SizedBox(height: 4),
          ...
          Row(
            children: [
              Expanded(
                  child: _buildTextButton(
                      '2', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      '3', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      '4', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      '5', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      '6', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      '7', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      '8', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      '9', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                  child: _buildTextButton(
                      ' ', controller, setState, onTextChanged)),
              const SizedBox(width: 4),
              Expanded(
                flex: 2,
                child: _buildActionButton('⌫', () {
  ...
  Future<void> _addToCart(int quantity) async {
    ...
    await _posController.addToCart(
      ...
      product:
          product, // ✅ NUEVO: Pasar producto completo para cálculo de impuestos
    );
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      subtitle: Text(
                          'Cantidad: ${product['quantity']} x ${CurrencyFormatter.formatCurrency(product['price'])}'),
                      trailing: Text(
