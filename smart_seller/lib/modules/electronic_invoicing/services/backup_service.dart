...
class BackupService {
  ...
  static Future<BackupResult> createManualBackup({
    ...
    try {
      ...
      final backupName = 'backup_${DateTime.now().year}_${DateTime.now().month.toString().padLeft(2, '0')}_${DateTime.now().day.toString().padLeft(2, '0')}_$timestamp';
  ...
  static Future<BackupResult> createAutoBackup() async {
    try {
      ...
      // ✅ Generar nombre del respaldo
      final timestamp = DateTime.now().millisecondsSinceEpoch;
      final backupName = 'auto_backup_${DateTime.now().year}_${DateTime.now().month.toString().padLeft(2, '0')}_${DateTime.now().day.toString().padLeft(2, '0')}_$timestamp';
      final backupPath = path.join(backupDir, backupName);
...
''';
  ...
  // ✅ Agregar a historial
  static Future<void> _addToHistory(String backupName, String backupPath, Map<String, dynamic> metadata) async {
    try {
      final prefs = await SharedPreferences.getInstance();
  ...
  static Future<void> _cleanupOldBackups() async {
    try {
      ...
      if (backupDirectories.length > maxBackups) {
        final toRemove = backupDirectories.take(backupDirectories.length - maxBackups);
  ...
  static Future<void> _restoreDatabase(String backupPath) async {
    try {
      ...
      if (await dbBackupFile.exists()) {
        ...
        print('📋 Script de restauración de BD cargado: ${backupContent.length} caracteres');
        
        // ✅ Aquí se ejecutaría el script SQL
        await Future.delayed(const Duration(seconds: 2)); // Simular restauración
  ...
  static Future<BackupIntegrityResult> verifyBackupIntegrity(String backupName) async {
    try {
      // ✅ Buscar respaldo en historial
      final history = await getBackupHistory();
      final backupEntry = history.firstWhere(
