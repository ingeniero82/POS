...
class PreEmissionValidationService {
  ...
  static Future<List<String>> validateSystemConfiguration() async {
    ...
    try {
      ...
      if (!config.isConsecutiveInRange(config.nextConsecutive)) {
        errors.add(
            'No hay consecutivos disponibles en el rango definido (${config.dianResolutionRange})');
      }
  ...
  static Future<List<String>> validateElectronicDocument(
      ElectronicDocument document) async {
    List<String> errors = [];
    ...
    if (document.items.isEmpty) {
      ...
      for (int i = 0; i < document.items.length; i++) {
        ...
        if (item.productName.isEmpty) {
          errors.add(
              'Item $itemIndex: La descripción del producto es obligatoria');
        }
    ...
    if ((document.total - calculatedTotal).abs() > 0.01) {
      errors.add(
          'El total no coincide con la suma de subtotal + impuestos - descuentos + cargos');
    }
  ...
  static Future<ValidationResult> validateBeforeEmission({
    ...
    try {
      ...
      if (!config.isConsecutiveInRange(config.nextConsecutive)) {
        allErrors.add(
            'No hay consecutivos disponibles. Consecutivo actual: ${config.currentConsecutive}, Rango: ${config.dianResolutionRange}');
      }
    } catch (e) {
  ...
  static List<String> _generateWarnings(Client client, List<Product> products) {
    ...
    if (client.phone?.isEmpty ?? true) {
      warnings.add(
          'El cliente no tiene teléfono registrado (recomendado para facturación)');
    }

    if (client.address?.isEmpty ?? true) {
      warnings.add(
          'El cliente no tiene dirección registrada (recomendado para facturación)');
    }

    if (client.city?.isEmpty ?? true) {
      warnings.add(
          'El cliente no tiene ciudad registrada (recomendado para facturación)');
    }

    if (client.department?.isEmpty ?? true) {
      warnings.add(
          'El cliente no tiene departamento registrado (recomendado para facturación)');
    }
    ...
    for (int i = 0; i < products.length; i++) {
      ...
      if (product.description.isEmpty && product.name.isNotEmpty) {
        warnings.add(
            'Producto $productIndex: Considerar agregar descripción detallada');
      }
