...
class PendingInvoiceQueueService {
    ...
    try {
      ...
      if (configJson != null) {
        ...
        _retryConfig = RetryConfig(
          ...
          initialDelay:
              Duration(minutes: configMap['initialDelayMinutes'] ?? 1),
  ...
  static Future<bool> addToQueue({
    ...
    try {
      ...
      if (existingIndex != -1) {
        ...
        final existingItem =
            PendingInvoiceItem.fromMap(jsonDecode(queueJson[existingIndex]));
        ...
        final updatedItem = existingItem.copyWith(
          ...
          nextRetryAt: error != null
              ? DateTime.now().add(_retryConfig
                  .calculateNextRetryDelay(existingItem.attemptCount + 1))
              : null,
        );
  ...
  static Future<List<PendingInvoiceItem>> getItemsByStatus(
      QueueStatus status) async {
    try {
    ...
    _autoRetryTimer = Timer.periodic(const Duration(minutes: 5), (timer) async {
      try {
        ...
        if (itemsToRetry.isNotEmpty) {
          print(
              '🔄 Reintentos automáticos: ${itemsToRetry.length} items disponibles');
    ...
    try {
      ...
      final updatedItem = item.copyWith(
        ...
        nextRetryAt: _retryConfig.shouldRetry(item.attemptCount, error)
            ? DateTime.now()
                .add(_retryConfig.calculateNextRetryDelay(item.attemptCount))
            : null,
      );
  ...
  static Future<Map<String, dynamic>> getQueueSummary() async {
    try {
      ...
      final itemsNeedingRetry =
          allItems.where((item) => item.canRetryNow).length;
      final itemsExhaustedRetries =
          allItems.where((item) => item.hasExhaustedRetries).length;
