...
class InvoiceStatusService {
  ...
  static Future<List<InvoiceStatusTracking>> getInvoiceStatusHistory(
    ...
    try {
      ...
      return trackingJson
          ...
          .toList()
        ..sort((a, b) =>
            b.timestamp.compareTo(a.timestamp)); // Más reciente primero
    } catch (e) {
  ...
  static Future<InvoiceStatus?> getCurrentInvoiceStatus(
  ...
  static Future<Map<InvoiceStatus, int>> getStatusStatistics() async {
    try {
      ...
      for (final invoice in allInvoices) {
        final status =
            await getCurrentInvoiceStatus(invoice.id?.toString() ?? '');
        if (status != null) {
  ...
  static Future<List<ElectronicDocument>> searchInvoices(
  ...
  static Future<bool> _updateInvoiceStatus(
    ...
    try {
      ...
      final prefs = await SharedPreferences.getInstance();
      final invoicesJson =
          allInvoices.map((inv) => jsonEncode(inv.toMap())).toList();
