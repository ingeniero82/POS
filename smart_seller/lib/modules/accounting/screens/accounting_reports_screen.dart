...
class _AccountingReportsScreenState extends State<AccountingReportsScreen> with SingleTickerProviderStateMixin {
  ...
  // ✅ NUEVO: Datos adicionales para reporte profesional
  Map<String, dynamic> _taxBreakdown = {};
  Map<String, double> _paymentMethods = {};
  Map<String, dynamic> _topData = {};
  ...
  Future<void> _exportAllReports() async {
    ...
    try {
      ...
      // ✅ NUEVO: Cargar datos adicionales para reporte profesional
      final taxBreakdown = await AccountingReportsService.getTaxAndDiscountsBreakdown(_fromDate, _toDate);
      final paymentMethods = await AccountingReportsService.getPaymentMethodBreakdown(_fromDate, _toDate);
      final topData = await AccountingReportsService.getTopProductsAndGroups(_fromDate, _toDate, limit: 10);
      ...
      // Guardar datos adicionales en variables de clase para usar en PDF
  ...
  Future<Uint8List> _generateCompleteReportPDF() async {
    ...
    // ✅ NUEVO: Calcular ventas netas
    final totalIncome = (_quickSummary['total_income'] as num?)?.toDouble() ?? 0.0;
    final totalDiscounts = (_taxBreakdown['totalDiscounts'] as num?)?.toDouble() ?? 0.0;
    final totalReturns = (_taxBreakdown['totalReturns'] as num?)?.toDouble() ?? 0.0;
    final netSales = totalIncome - totalDiscounts - totalReturns;
      ...
      pw.MultiPage(
        ...
        build: (pw.Context context) {
          return [
            ...
            pw.SizedBox(height: 12),
            pw.Text('VENTAS POR TIPO DE IVA', style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold)),
            pw.SizedBox(height: 8),
            pw.Table(
              columnWidths: {0: const pw.FlexColumnWidth(3), 1: const pw.FlexColumnWidth(1)},
              children: [
                _buildPDFTableRow('Ventas Exentas', (_taxBreakdown['exemptAmount'] as num?)?.toDouble() ?? 0.0),
                _buildPDFTableRow('Ventas Excluidas', (_taxBreakdown['excludedAmount'] as num?)?.toDouble() ?? 0.0),
                _buildPDFTableRow('Ventas Gravadas', (_taxBreakdown['taxedAmount'] as num?)?.toDouble() ?? 0.0),
              ],
            ),
            pw.SizedBox(height: 12),
            pw.Text('IVA POR TASAS', style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold)),
            pw.SizedBox(height: 8),
            pw.Table(
              columnWidths: {0: const pw.FlexColumnWidth(3), 1: const pw.FlexColumnWidth(1)},
              children: [
                _buildPDFTableRow('IVA 0%', (_taxBreakdown['vatAt0'] as num?)?.toDouble() ?? 0.0),
                _buildPDFTableRow('IVA 5%', (_taxBreakdown['vatAt5'] as num?)?.toDouble() ?? 0.0),
                _buildPDFTableRow('IVA 19%', (_taxBreakdown['vatAt19'] as num?)?.toDouble() ?? 0.0),
                _buildPDFTableRow('Total IVA', (_taxBreakdown['totalVat'] as num?)?.toDouble() ?? 0.0, isImportant: true),
              ],
            ),
            // Otros impuestos (si aplican)
            ...() {
              final ipoConsumo = (_taxBreakdown['ipoConsumoAmount'] as num?)?.toDouble() ?? 0.0;
              final plasticBagTax = (_taxBreakdown['plasticBagTaxAmount'] as num?)?.toDouble() ?? 0.0;
              final List<pw.Widget> widgets = [];
              
              if (ipoConsumo > 0) {
                widgets.addAll([
                  pw.SizedBox(height: 12),
                  pw.Text('OTROS IMPUESTOS', style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold)),
                  pw.SizedBox(height: 8),
                  pw.Table(
                    columnWidths: {0: const pw.FlexColumnWidth(3), 1: const pw.FlexColumnWidth(1)},
                    children: [
                      _buildPDFTableRow('IpoConsumo', ipoConsumo),
                    ],
                  ),
                ]);
              }
              ...
              if (plasticBagTax > 0) {
                widgets.addAll([
                  ...
                  pw.Table(
                    columnWidths: {0: const pw.FlexColumnWidth(3), 1: const pw.FlexColumnWidth(1)},
                    children: [
                      _buildPDFTableRow('Impuesto Bolsas Plásticas', plasticBagTax),
                      _buildPDFTableRow('Cantidad de Bolsas', (_taxBreakdown['plasticBagCount'] as num?)?.toDouble() ?? 0.0),
                    ],
                  ),
                ]);
              }
            ...
            pw.Table(
              columnWidths: {0: const pw.FlexColumnWidth(3), 1: const pw.FlexColumnWidth(1)},
              children: [
                _buildPDFTableRow('Subtotal Antes de Impuestos', (_taxBreakdown['subtotalBeforeTaxes'] as num?)?.toDouble() ?? 0.0),
              ],
            ),
            ...
            // ========== 6. DESCUENTOS Y DEVOLUCIONES ==========
            pw.Text(
              '6. DESCUENTOS Y DEVOLUCIONES',
              style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold, color: PdfColors.blue900),
            ),
            pw.SizedBox(height: 12),
            _buildPDFCard('Total Descuentos Aplicados', (_taxBreakdown['totalDiscounts'] as num?)?.toDouble() ?? 0.0),
            pw.SizedBox(height: 8),
            _buildPDFCard('Total Devoluciones', (_taxBreakdown['totalReturns'] as num?)?.toDouble() ?? 0.0),
            pw.SizedBox(height: 8),
            _buildPDFCard('Transacciones con Devolución', (_taxBreakdown['returnTransactions'] as num?)?.toDouble() ?? 0.0),
            pw.SizedBox(height: 8),
            _buildPDFCard('Ventas Netas (Bruto - Descuentos - Devoluciones)', netSales, isImportant: true),
              ...
              pw.Table(
                ...
                children: [
                  for (final entry in _paymentMethods.entries)
                    _buildPDFTableRow(entry.key, entry.value),
                  _buildPDFTableRow('Total', _paymentMethods.values.fold(0.0, (sum, val) => sum + val), isImportant: true),
                ],
              ),
            ] else ...[
              pw.Text('No hay datos de métodos de pago', style: pw.TextStyle(fontSize: 10, color: PdfColors.grey600)),
            ],
            ...
            // ========== 8. TOP PRODUCTOS Y GRUPOS ==========
            pw.Text(
              '8. ANÁLISIS DE PRODUCTOS Y GRUPOS',
              style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold, color: PdfColors.blue900),
            ),
            pw.SizedBox(height: 12),
            if (_topData['topProducts'] != null && (_topData['topProducts'] as List).isNotEmpty) ...[
              pw.Text('TOP 10 PRODUCTOS MÁS VENDIDOS', style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold)),
              pw.SizedBox(height: 8),
              pw.Table(
                columnWidths: {
                  0: const pw.FlexColumnWidth(2),
                ...
                children: [
                  pw.TableRow(
                    decoration: const pw.BoxDecoration(color: PdfColors.grey200),
                    children: [
                      _buildPDFTableCell('Producto', isHeader: true),
                      _buildPDFTableCell('Cantidad', isHeader: true),
                      _buildPDFTableCell('Total', isHeader: true),
                    ],
                  ),
                  for (final product in (_topData['topProducts'] as List).take(10))
                    pw.TableRow(
                      children: [
                        pw.Padding(
                          padding: const pw.EdgeInsets.all(6),
                          child: pw.Text((product['name'] as String?) ?? '', style: const pw.TextStyle(fontSize: 9)),
                        ),
                        pw.Padding(
                          padding: const pw.EdgeInsets.all(6),
                          child: pw.Text('${product['quantity'] ?? 0}', style: const pw.TextStyle(fontSize: 9)),
                        ),
                        pw.Padding(
                          padding: const pw.EdgeInsets.all(6),
                          child: pw.Text(_currencyFormat.format((product['amount'] as num?)?.toDouble() ?? 0.0), style: const pw.TextStyle(fontSize: 9)),
                        ),
                      ],
                    ),
                ],
            ...
            if (_topData['topGroups'] != null && (_topData['topGroups'] as List).isNotEmpty) ...[
              pw.Text('TOP 10 GRUPOS/CATEGORÍAS MÁS VENDIDOS', style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold)),
              pw.SizedBox(height: 8),
              pw.Table(
                columnWidths: {
                  0: const pw.FlexColumnWidth(2),
                ...
                children: [
                  pw.TableRow(
                    decoration: const pw.BoxDecoration(color: PdfColors.grey200),
                    children: [
                      _buildPDFTableCell('Grupo', isHeader: true),
                      _buildPDFTableCell('Cantidad', isHeader: true),
                      _buildPDFTableCell('Total', isHeader: true),
                    ],
                  ),
                  for (final group in (_topData['topGroups'] as List).take(10))
                    pw.TableRow(
                      children: [
                        pw.Padding(
                          padding: const pw.EdgeInsets.all(6),
                          child: pw.Text((group['name'] as String?) ?? '', style: const pw.TextStyle(fontSize: 9)),
                        ),
                        pw.Padding(
                          padding: const pw.EdgeInsets.all(6),
                          child: pw.Text('${group['quantity'] ?? 0}', style: const pw.TextStyle(fontSize: 9)),
                        ),
                        pw.Padding(
                          padding: const pw.EdgeInsets.all(6),
                          child: pw.Text(_currencyFormat.format((group['amount'] as num?)?.toDouble() ?? 0.0), style: const pw.TextStyle(fontSize: 9)),
                        ),
                      ],
                    ),
                ],
  ...
  pw.TableRow _buildPDFTableRow(String label, double value, {bool isImportant = false}) {
    return pw.TableRow(
      children: [
        pw.Padding(
          padding: const pw.EdgeInsets.all(8),
          child: pw.Text(
            label,
            style: pw.TextStyle(fontWeight: isImportant ? pw.FontWeight.bold : pw.FontWeight.normal),
          ),
        ),
        pw.Padding(
          padding: const pw.EdgeInsets.all(8),
