...
class _AccountingReportsScreenState extends State<AccountingReportsScreen>
  ...
  Future<void> _exportAllReports() async {
    ...
    try {
      ...
      final taxBreakdown =
          await AccountingReportsService.getTaxAndDiscountsBreakdown(
              _fromDate, _toDate);
      final paymentMethods =
          await AccountingReportsService.getPaymentMethodBreakdown(
              _fromDate, _toDate);
      final topData = await AccountingReportsService.getTopProductsAndGroups(
          _fromDate, _toDate,
          limit: 10);
      ...
      pw.MultiPage(
        ...
        build: (pw.Context context) {
          return [
            ...
            pw.SizedBox(height: 12),
            _buildPDFCard(
                'Total Transacciones', report.totalTransactions.toDouble()),
            pw.SizedBox(height: 8),
  ...
  Future<Uint8List> _generateCompleteReportPDF() async {
    ...
    final totalIncome =
        (_quickSummary['total_income'] as num?)?.toDouble() ?? 0.0;
    final totalDiscounts =
        (_taxBreakdown['totalDiscounts'] as num?)?.toDouble() ?? 0.0;
    final totalReturns =
        (_taxBreakdown['totalReturns'] as num?)?.toDouble() ?? 0.0;
        ...
        build: (pw.Context context) {
          return [
            ...
            _buildPDFCard('Utilidad Neta',
                ...
                isImportant: true),
            pw.SizedBox(height: 8),
            _buildPDFCard('Total Transacciones',
                (_quickSummary['transaction_count'] as num? ?? 0).toDouble()),
            ...
            pw.SizedBox(height: 12),
            pw.Text('VENTAS POR TIPO DE IVA',
                style:
            ...
            pw.Table(
              ...
              children: [
                _buildPDFTableRow('Ventas Exentas',
                    (_taxBreakdown['exemptAmount'] as num?)?.toDouble() ?? 0.0),
                _buildPDFTableRow(
                    'Ventas Excluidas',
                    (_taxBreakdown['excludedAmount'] as num?)?.toDouble() ??
                        0.0),
                _buildPDFTableRow('Ventas Gravadas',
                    (_taxBreakdown['taxedAmount'] as num?)?.toDouble() ?? 0.0),
              ],
            ),
            pw.SizedBox(height: 12),
            pw.Text('IVA POR TASAS',
                style:
                    pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold)),
            pw.SizedBox(height: 8),
            pw.Table(
              ...
              children: [
                _buildPDFTableRow('IVA 0%',
                    (_taxBreakdown['vatAt0'] as num?)?.toDouble() ?? 0.0),
                _buildPDFTableRow('IVA 5%',
                    (_taxBreakdown['vatAt5'] as num?)?.toDouble() ?? 0.0),
                _buildPDFTableRow('IVA 19%',
                    (_taxBreakdown['vatAt19'] as num?)?.toDouble() ?? 0.0),
                _buildPDFTableRow('Total IVA',
                    (_taxBreakdown['totalVat'] as num?)?.toDouble() ?? 0.0,
                    isImportant: true),
              ],
            ),
            ...
            ...() {
              final ipoConsumo =
                  (_taxBreakdown['ipoConsumoAmount'] as num?)?.toDouble() ??
                      0.0;
              final plasticBagTax =
                  (_taxBreakdown['plasticBagTaxAmount'] as num?)?.toDouble() ??
                      0.0;
              ...
              if (plasticBagTax > 0) {
                widgets.addAll([
                  ...
                  pw.Table(
                    ...
                    children: [
                      _buildPDFTableRow(
                          'Impuesto Bolsas Plásticas', plasticBagTax),
                      _buildPDFTableRow(
                          'Cantidad de Bolsas',
                          (_taxBreakdown['plasticBagCount'] as num?)
                                  ?.toDouble() ??
                              0.0),
                    ],
                  ),
            ...
            pw.Table(
              ...
              children: [
                _buildPDFTableRow(
                    ...
                    (_taxBreakdown['subtotalBeforeTaxes'] as num?)
                            ?.toDouble() ??
                        0.0),
              ],
            ),
            ...
            pw.SizedBox(height: 12),
            _buildPDFCard('Total Descuentos Aplicados',
                (_taxBreakdown['totalDiscounts'] as num?)?.toDouble() ?? 0.0),
            pw.SizedBox(height: 8),
            _buildPDFCard('Total Devoluciones',
                (_taxBreakdown['totalReturns'] as num?)?.toDouble() ?? 0.0),
            pw.SizedBox(height: 8),
            _buildPDFCard(
                'Transacciones con Devolución',
                (_taxBreakdown['returnTransactions'] as num?)?.toDouble() ??
                    0.0),
            pw.SizedBox(height: 8),
            _buildPDFCard(
                'Ventas Netas (Bruto - Descuentos - Devoluciones)', netSales,
                isImportant: true),
            ...
            if (_paymentMethods.isNotEmpty) ...[
              pw.Table(
                ...
                children: [
                  ...
                  _buildPDFTableRow('Total',
                      _paymentMethods.values.fold(0.0, (sum, val) => sum + val),
                      isImportant: true),
                ],
              ),
            ] else ...[
              pw.Text('No hay datos de métodos de pago',
                  style: const pw.TextStyle(
            ...
            pw.SizedBox(height: 12),
            if (_topData['topProducts'] != null &&
                (_topData['topProducts'] as List).isNotEmpty) ...[
              ...
              pw.Table(
                ...
                children: [
                  ...
                  for (final product
                      in (_topData['topProducts'] as List).take(10))
                    pw.TableRow(
                      children: [
                        pw.Padding(
                          padding: const pw.EdgeInsets.all(6),
                        ...
                        pw.Padding(
                          ...
                          child: pw.Text(
                              _currencyFormat.format(
                                  (product['amount'] as num?)?.toDouble() ??
                                      0.0),
                              style: const pw.TextStyle(fontSize: 9)),
                        ),
                      ],
            ...
            if (_topData['topGroups'] != null &&
                (_topData['topGroups'] as List).isNotEmpty) ...[
              pw.Text('TOP 10 GRUPOS/CATEGORÍAS MÁS VENDIDOS',
                  style: pw.TextStyle(
                      fontSize: 12, fontWeight: pw.FontWeight.bold)),
              pw.SizedBox(height: 8),
              pw.Table(
                  ...
                  for (final group in (_topData['topGroups'] as List).take(10))
                    pw.TableRow(
                      children: [
                        pw.Padding(
                          padding: const pw.EdgeInsets.all(6),
                          child: pw.Text((group['name'] as String?) ?? '',
                              style: const pw.TextStyle(fontSize: 9)),
                        ),
                        pw.Padding(
                          padding: const pw.EdgeInsets.all(6),
                          child: pw.Text('${group['quantity'] ?? 0}',
                              style: const pw.TextStyle(fontSize: 9)),
                        ),
                        pw.Padding(
                          padding: const pw.EdgeInsets.all(6),
                          child: pw.Text(
                              _currencyFormat.format(
                                  (group['amount'] as num?)?.toDouble() ?? 0.0),
                              style: const pw.TextStyle(fontSize: 9)),
                        ),
                      ],
                    ),
