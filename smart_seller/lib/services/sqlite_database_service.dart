...
// Crear las tablas
  ...
  static Future<void> _onUpgrade(
    ...
    if (oldVersion < 4) {
      print(
          '🔧 Agregando campos de IVA, IpoConsumo, bolsas y recogidas de efectivo...');
  ...
  static Future<void> migrateAddTaxAndPickupFields(Database db) async {
    try {
      ...
      for (final column in groupColumns) {
        try {
          await db.execute(
              'ALTER TABLE groups ADD COLUMN ${column['name']} ${column['type']}');
  ...
  static Future<void> _createDefaultUser() async {
    ...
    if (supervisorExists.isEmpty) {
      ...
      print(
          '✅ Usuario supervisor creado con contraseña segura: supervisor / 123456');
    }
  }
  ...
  static Future<List<Map<String, String>>> getMostFrequentProductNames(
      {int days = 30, int limit = 12}) async {
    try {
        ...
        if (!sale.isReturn) {
          for (final item in sale.items) {
            ...
            productFrequency[key] =
                (productFrequency[key] ?? 0) + item.quantity;
          }
        }
      }
  ...
  static Future<List<Sale>> getSales(
    ...
    return results.map((saleData) {
      final sale = Sale(
        ...
        ipoConsumoAmount:
            (saleData['ipoConsumoAmount'] as num?)?.toDouble() ?? 0.0,
        plasticBagTaxAmount:
            (saleData['plasticBagTaxAmount'] as num?)?.toDouble() ?? 0.0,
        ...
        if (itemsString != null && itemsString.isNotEmpty) {
          ...
          sale.items = itemsList
              .map((item) => SaleItem(
                    ...
                    ipoConsumoRate: item['ipoConsumoRate'] != null
                        ? (item['ipoConsumoRate'] as num).toDouble()
                        : null,
                    ipoConsumoType: item['ipoConsumoType'],
                    itemIpoConsumo: (item['itemIpoConsumo'] ?? 0.0).toDouble(),
                    isPlasticBag: item['isPlasticBag'] == true,
                    plasticBagTax: item['plasticBagTax'] != null
                        ? (item['plasticBagTax'] as num).toDouble()
                        : null,
  ...
  static Future<List<CashPickup>> getCashPickupsBySession(
